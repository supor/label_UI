...
def stream_check():
    ...
    sleep(config.STIME)
    _driver.get("https://www.zzcrowd.com/checker/flow-select")
    common.find_by_xpath(_driver, '//*[@id="subjob-list-body"]/table/tbody/tr/td[6]/a')
    for i in range(20000):
        common.find_by_id(_driver, 'next2')
        logger.write_debug(u"第%s个任务已检查完成" % i)

    # sleep(config.STIME)
    #
    # logger.write_debug(u"点击检查，进入检查员管理页面")
    # common.find_by_xpath(_driver, '//*[@id="main"]/div/div/div/div[2]/div')
    #
    ...
    # common.find_by_xpath(_driver, '/html/body/div[2]/div/div/div[3]/button')
    #
