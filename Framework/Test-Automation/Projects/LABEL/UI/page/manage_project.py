...
def stream_assign():
    global _driver
    if config.BASE_URL == config.BASE_URL_VENDOR:
        _driver = common.login(config.USERNAME_VENDOR[0], config.PWD_VENDOR)
    if config.BASE_URL == config.BASE_URL_DEV:
        _driver = common.login(config.USERNAME_DEV[0], config.PWD_DEV[0])
